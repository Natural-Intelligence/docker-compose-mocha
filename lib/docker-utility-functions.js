...
const cleanupContainersByEnvironmentName = coroutine(
  function* cleanupContainersByEnvironmentName(envName/* : string */,
    pathToComposeFile/* : string */, envDisplayName/* : string */) {
    const consoleMessage = `Stopping all containers of environment codenamed: ${envDisplayName}.. `;
    ...
    yield exec(`docker-compose -p ${envName} -f "${pathToComposeFile}" down`);
